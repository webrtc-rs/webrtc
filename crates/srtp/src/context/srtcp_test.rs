...
mod test {
    ...
    fn test_rtcp_invalid_auth_tag() -> Result<(), Error> {
        ...
        let mut rtcp_packet = RTCP_TEST_CASES[0].encrypted.clone();
    ...
    fn test_rtcp_replay_detector_separation() -> Result<(), Error> {
        let mut decrypt_context = Context::new(
            ...
            ProtectionProfile::AES128CMHMACSHA1_80,
            None,
            Some(option::srtcp_replay_protection(10)),
        )?;
    ...
    fn get_rtcp_index(encrypted: &[u8], auth_tag_len: usize) -> u32 {
        let tail_offset = encrypted.len() - (auth_tag_len + context::SRTCP_INDEX_SIZE);
        let mut reader =
            Cursor::new(&encrypted[tail_offset..tail_offset + context::SRTCP_INDEX_SIZE]);
        let rtcp_index = reader.read_u32::<BigEndian>().unwrap() & 0x7FFFFFFF; //^(1 << 31)
        rtcp_index
    }
