...
//         let res = pck.payload(1, BytesMut::new());
...
//         let result = pkt.unmarshal(&mut BytesMut::new());
//         assert!(result.is_err(), "Unmarshal did not fail on nil payload");

//         let result = pkt.unmarshal(&mut [0x00u8, 0x00][..].into());
//         assert!(
//             result.is_err(),
//             "Unmarshal accepted a packet that is too small for a payload and header"
//         );

//         let result = pkt.unmarshal(&mut [0xFF, 0x00, 0x00][..].into());
//         assert!(
//             result.is_err(),
//             "Unmarshal accepted a packet with a NALU Type we don't handle"
//         );

//         let result = pkt.unmarshal(&mut incomplete_single_payload_multi_nalu.as_slice().into());
//         assert!(
//             result.is_err(),
//             "Unmarshal accepted a STAP-A packet with insufficient data"
//         );

//         let res = pkt.unmarshal(&mut single_payload.as_slice().into())?;
//         assert_eq!(
...
//             let res = pkt.unmarshal(&mut p.as_slice().into())?;
...
//         let res = pkt.unmarshal(&mut single_payload_multi_nalu.as_slice().into())?;
//         assert_eq!(
