...
//         // Empty packet
//         let result = pck.unmarshal(&mut BytesMut::new());
//         assert_eq!(
//             result.err(),
...
//         let result = pck.unmarshal(&mut small_bytes[..].into());
//         assert_eq!(
//             result.err(),
//             Some(RTPError::ShortPacket),
//             "Result should be err in case of error"
...
//         let result = pck.unmarshal(&mut raw_bytes[..].into())?;
...
//         let raw_bytes = vec![0x80, 0x80, 0x00, 0x00];
//         let result = pck.unmarshal(&mut raw_bytes[..].into())?;
//         assert!(!result.is_empty(), "Payload must be not empty");
...
//         let raw_bytes = vec![0xff, 0xff, 0x00, 0x00];
//         let result = pck.unmarshal(&mut raw_bytes[..].into());
//         assert_eq!(
//             result.err(),
//             Some(RTPError::ShortPacket),
...
//         assert!(
//             !checker.is_partition_head([0x00][..].into()),
//             "Small packet should not be the head of a new partition"
//         );

//         assert!(
//             checker.is_partition_head([0x10, 0x00, 0x00, 0x00][..].into()),
//             "Packet with S flag should be the head of a new partition"
//         );

//         assert!(
//             !checker.is_partition_head([0x00, 0x00, 0x00, 0x00][..].into()),
