...
mod test {
    ...
    fn test_packetizer() {
        ...
        let packets = packetizer.packetize(multiplepayload, 2000).unwrap();
    ...
    fn test_packetizer_abs_send_time() {
        ...
        let payload = &mut [0x11u8, 0x12, 0x13, 0x14];

        let packets = pktizer.packetize(payload, 2000).unwrap();
