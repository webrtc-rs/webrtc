...
func (i *chunkInit) unmarshal(raw []byte) error {
    ...
    } else if len(i.raw) < INIT_CHUNK_MIN_LENGTH {
        return fmt.Errorf("%w: %d actual: %d", errChunkValueNotLongEnough, INIT_CHUNK_MIN_LENGTH, len(i.raw))
    }
...
func (i *chunkInit) check() (abort bool, err error) {
    ...
    // ACK.
    if i.advertised_receiver_window_credit < 1500 {
        abort = true
