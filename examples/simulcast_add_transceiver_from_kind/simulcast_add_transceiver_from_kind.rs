...
struct Handler {
    ...
    output_tracks: Arc<Mutex<HashMap<String, (Arc<dyn TrackLocal>, u32)>>>,
}
...
impl PeerConnectionEventHandler for Handler {
    ...
    async fn on_track(&self, track: Arc<dyn TrackRemote>) {
        ...
        self.runtime.spawn(Box::pin(async move {
            ...
            let mut ssrc_map: HashMap<u32, (Arc<dyn TrackLocal>, u32)> = HashMap::new();
            ...
            while let Some(evt) = track.poll().await {
                match evt {
                    TrackRemoteEvent::OnOpen(init) => {
                        ...
                        let output_tracks = output_tracks.lock().await;
...
async fn async_main() -> Result<()> {
    ...
    let output_tracks = Arc::new(Mutex::new(HashMap::new()));
    ...
    let handler = Arc::new(Handler {
        ...
        output_tracks: output_tracks.clone(),
    });
    ...
    for rid in ["q", "h", "f"] {
        ...
        let mut output_tracks = output_tracks.lock().await;
        output_tracks.insert(
            rid.to_owned(),
            (
                Arc::clone(&transceiver.sender().await?.unwrap().track()),
                ssrc,
            ),
        );
    }
