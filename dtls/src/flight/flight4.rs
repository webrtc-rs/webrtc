...
pub(crate) async fn flight4parse<C: FlightConn>(
    /*context.Context,*/
    c: C,
    state: &mut State,
            ...
            if let Some(cipher_suite) = &state.cipher_suite {
                if state.extended_master_secret {
                    let session_hash = match cache
                        .session_hash(cipher_suite.hash_func(), cfg.initial_epoch, &[])
                        .await
                    {
                        Ok(s) => s,
                        Err(err) => {
    ...
    if cfg.local_psk_callback.is_none() {
        ...
        pkts.push(Packet {
            record: RecordLayer {
                ...
                content: Content::Handshake(Handshake {
                    handshake_header: HandshakeHeader::default(),
                    handshake_message: HandshakeMessage::Certificate(HandshakeMessageCertificate {
